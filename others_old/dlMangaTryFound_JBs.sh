...
if [ "$newFolder" != 'n' ]; then
    ...
    cd "$nameManga" || exit # Move to there
fi
...
while [ "$chapterStart" -lt "$chapterEnd" ]; do # Run until chapter download equal to end chapter to download
    ...
    cd "$nameFolder" || exit # Move to download folder
    ...
    cd .. || exit # Move to folder up, leaving the chapter folder
...
if [ "$newFolder" != 'n' ]; then
    cd .. || exit # Move to folder up, leaving the manga folder
fi
